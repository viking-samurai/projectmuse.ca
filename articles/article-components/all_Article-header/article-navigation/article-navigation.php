<nav id="nav">
	<ul>
		<li>
			<a class="home-nav-button" href="../index.php">Home</a>
		</li>
		<li>
			<a class="current" href="../index.php#intro">Services</a>
			<ul>
				<li class="main-nav-dropdown">
					<a href="../services/custom-built-creations.php">Custom-built Creations</a>
				</li>
				<li class="main-nav-dropdown">
					<a href="../services/seo-and-data-packages.php">SEO and Data Packages</a>
				</li>
				<li class="main-nav-dropdown">
					<a href="../services/management-and-consulting.php">Management and Consulting</a>
				</li>
			</ul>
		</li>
		<li>
			<a href="../index.php#main">Portfolio</a>
			<ul>
				<li class="main-nav-dropdown">
					<a href="../index.php#blogs-and-small-business">Blogs and Small Business Sites</a>
				</li>
				<li class="main-nav-dropdown">
					<a href="../index.php#commercial-and-industrial">Industrial and Commercial Sites</a>
				</li>
				<li class="main-nav-dropdown">
					<a href="../index.php#resume-websites">Resumé Websites and Curriculum Vitae</a>
				</li>
				<li class="main-nav-dropdown">
					<a href="../index.php#self-publishing">Self Publishing and Other Sites</a>
				</li>
			</ul>
		</li>
		<li>
			<a href="../index.php#blog">Blog</a>
			<ul>
				<li class="main-nav-dropdown">
					<a href="../index.php#persistence">Essay on Persistance</a>
				</li>
				<li class="main-nav-dropdown">
					<a href="../index.php#creativity">Essay on Creativity</a>
				</li>
				<li class="main-nav-dropdown">
					<a href="../index.php#grinding">Essay on Grinding</a>
				</li>
				<li class="main-nav-dropdown">
					<a href="../index.php#economics">Essay on Economics</a>
				</li>
			</ul>
		</li>
		<li>
			<a href="../index.php#footer">Contact</a>
		</li>
	</ul>
</nav>